...
public class ProductWebObject extends DefaultObject {
    ...
    public String computePrice(@PathParam("productName") String productName) {
        DocumentModel workflowInstance;
        	String query = "SELECT * FROM product WHERE ecm:mixinType != 'HiddenInNavigation' AND ecm:isProxy = 0 AND ecm:isVersion = 0 AND ecm:isTrashed = 0 AND productSchema:productName = 'ChocoStick'";
